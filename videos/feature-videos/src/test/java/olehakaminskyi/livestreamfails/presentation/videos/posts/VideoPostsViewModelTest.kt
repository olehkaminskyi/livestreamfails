...
class VideoPostsViewModelTest {
    ...
    fun setup() = coroutinesTestRule.testDispatcher.runBlockingTest {
        ...
        viewModel = VideoPostsViewModel(videoPostsRepository)
    }
    @Test
    ...
    fun refresh() = runBlockingTest {
        val result = listOf(VideoPost(1, "url"))
        val valuesVerifier = viewModel.progress
            .valuesVerifier(Progress.NONE, Progress.LOADING, Progress.NONE)
        whenever(videoPostsRepository.refresh()).thenReturn(DataResult(result))
        viewModel.refresh()
